...
(weapon-to-string (assoc (completing-read "Arme" weapons-list) weapons-list))
#+end_src
...
(defun weapon-get-damage-from-string (weapon)
  (let ((specs (split-string (nth 2 weapon) "+")))
    (--map (roll-from-string it) specs)))
#+end_src

#+RESULTS:
: weapon-get-damage-from-string
